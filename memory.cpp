...
struct Record {
    ...
    void leak_msg() {
        std::cout << "  " << info;
        std::cout << ": " << size << " bytes allocated here were never freed." << std::endl;
    }
};
...
struct MemTrack {
    ...
    ~MemTrack() {
        // free all memory for reals now
        for (void *ptr : all_allocs) {
            free(ptr);
        }
        ...
        std::cout << "  leaked: " << alloced << " bytes in " << allocated_records.size() << " allocations" << std::endl;
        std::cout << std::endl;
    ...
    void check_double_free(void *ptr) {
        for (size_t i = 0; i < freed_records.size(); ++i) {
            if (freed_records.at(i).ptr == ptr) {
                ...
                std::cout << "  attempted to free pointer " << ptr << " twice." << std::endl;
                std::cout << std::endl;
